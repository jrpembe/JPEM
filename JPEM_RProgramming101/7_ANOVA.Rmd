...
---
H0: The amount of REM sleep for all vores is the same
H1: The amount of REM sleep for all vores is not the same

```{r Sleep Patterns of Mammals}

my_data <- msleep %>% 
  select(vore, sleep_rem) %>% 
  drop_na()

model1 <- aov(sleep_rem ~ vore, data = my_data)

summary(model1)

# The p-value is < 0.05 so we reject the null hypothesis (mean REM sleep is the same)

# alternate method using pipes
...
```{r}
# Create a density plot for sleep_rem by vore
...
msleep %>% 
  ...
  labs(title = "Density Plot of REM Sleep by Vore Type",
       x = "REM Sleep (hours)",
       y = "Density") +
  theme_minimal()
